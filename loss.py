...
def dice_bce_loss(inputs, targets, smooth=1e-6, bce_weight=0.5):
    """Combined Dice loss and BCE loss."""
    probs = torch.sigmoid(inputs)
    dims = tuple(range(1, targets.ndimension()))
    intersection = (probs * targets).sum(dim=dims)
    dice_score = (2 * intersection + smooth) / (probs.sum(dim=dims) + targets.sum(dim=dims) + smooth)
    dice_loss = 1 - dice_score
    bce = F.binary_cross_entropy_with_logits(inputs, targets, reduction='none')
    bce = bce.mean(dim=dims)
    return bce * bce_weight + dice_loss * (1 - bce_weight)


def lovasz_loss(inputs, targets, classes='present', per_image=False, ignore=None):
    """Lovasz loss for imbalanced segmentation."""
    probs = torch.sigmoid(inputs)
    dims = tuple(range(1, targets.ndimension()))
    intersection = (probs * targets).sum(dim=dims)
    ...
    if classes == 'present':
        ...
        raise ValueError("Invalid value for 'classes'. Use 'present' or 'all'.")
    if per_image:
