...
class SEModule(nnx.Module):
    def __init__(self, in_size, out_size, rngs: nnx.Rngs):
        # gap
        self.line1 = nnx.Conv(in_features=in_size, out_features=in_size//4, kernel_size=(1, 1), strides=1, padding="same", use_bias=False, rngs=rngs)
        self.relu6 = nnx.Activation(relu6)
        self.line2 = nnx.Conv(in_features=in_size//4, out_features=out_size, kernel_size=(1, 1), strides=1, padding="same", use_bias=False, rngs=rngs)
...
class MobileNetV3Small(nnx.Module):
    ...
    def __init__(self, scale, out_channels, rngs: nnx.Rngs):
        ...
        # # k  in    exp     out      NL          s
        # [3,  16,    16,     16,     nnx.relu,    1],
        # [3,  16,    72,     24,     nnx.relu,    2],
        # [3,  24,    88,     24,     nnx.relu,    1],
        # [5,  24,    96,     40,     nnx.relu,    2],
        # [5,  40,    240,    40,     nnx.relu,    1],
        # [5,  40,    120,    48,     nnx.relu,    1],
        # [5,  48,    144,    48,     nnx.relu,    1],
        self.bneck0 = BottleNeck(16, 16, 16, 2, 3, nnx.relu, scale, rngs) # Todo: Add SE
...
class TinyLPR(nnx.Module):
    ...
    def __init__(self, time_steps, n_class, n_feat, rngs: nnx.Rngs):
        self.backbone = MobileNetV3Small(0.35, n_feat, rngs)
